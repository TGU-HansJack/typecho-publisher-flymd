...
function xmlParseResponse(text) {
  ...
  if (fault) {
    ...
    const msg = (obj && (obj.faultString || obj['faultString'])) || 'XML-RPC éè¯¯'
...
function openOverlay(title, contentBuilder) {
  ...
  dialog.innerHTML = `<div class="tp-fly-header"><div>${title}</div><button class="tp-fly-btn" id="tp-close">Ã</button></div><div class="tp-fly-body"></div>`
...
export async function openSettings(ctx) {
  ...
  openOverlay('Typecho åå¸å¨è®¾ç½?, (body, close) => {
    const wrap = document.createElement('div')
    wrap.className = 'tp-fly-grid'
    wrap.innerHTML = `
      <label>æ¥å£ URL</label><input id="tp-endpoint" type="url" placeholder="https://your-site.com/xmlrpc.php" value="${s.endpoint || ''}">
      <label>ç¨æ·å?/label><input id="tp-username" type="text" value="${s.username || ''}">
      <label>å¯ç </label><input id="tp-password" type="password" value="${s.password || ''}">
      <label>é»è®¤åå®¢ID</label><input id="tp-blogid" type="text" value="${s.blogId || '0'}">
      <label>CORS ä»£ç</label><input id="tp-proxy" type="url" placeholder="å¯å« {target}" value="${s.proxyUrl || ''}">
      <div class="tp-fly-rowfull">å»ºè®®ä¼åä½¿ç¨æ¡é¢ç¯å¢ï¼åç½®åçç½ç»å±ï¼é¿å?CORSï¼å¦éæµè§å¨åä½¿ç¨ï¼è¯·éç½®èªå»ºä»£çã?/div>
      <label>ä½¿ç¨å½åæ¶é´</label><input id="tp-usecur" type="checkbox" ${s.useCurrentTime ? 'checked' : ''}>
      <label>åå¸æ¶é´åç§»(å°æ¶)</label><input id="tp-offset" type="text" value="${s.publishTimeOffset || 0}">
    `
    ...
    btnSave.addEventListener('click', async () => {
      ...
      try { ctx.ui?.notice?.('è®¾ç½®å·²ä¿å­?, 'ok', 1500) } catch {}
      close()
    })
...
async function openPublishDialog(ctx) {
  ...
  if (!settings.endpoint || !settings.username || !settings.password) {
    const go = await ctx.ui?.confirm?.('å°æªéç½® Typecho åæ°ï¼æ¯å¦ç°å¨è®¾ç½®ï¼')
  ...
  const preset = {
    title: (fm[K.title] || '').toString() || 'æªå½åææ¡?,
  ...
  openOverlay('åå¸å?Typecho', (bodyEl, close) => {
    const wrap = document.createElement('div')
    wrap.className = 'tp-fly-grid'
    wrap.innerHTML = `
      <label>æ é¢</label><input id="tp-title" type="text" value="${preset.title}">
      <label>Slug</label><input id="tp-slug" type="text" placeholder="å¯çç©? value="${preset.slug || ''}">
      <label>æ ç­¾</label><input id="tp-tags" type="text" placeholder="éå·åé" value="${preset.tags.join(', ')}">
      <label>åç±»</label><input id="tp-cats" type="text" placeholder="éå·åéï¼è³å°?1 ä¸? value="${preset.categories.join(', ')}">
      <label>èç¨¿</label><input id="tp-draft" type="checkbox" ${preset.draft ? 'checked' : ''}>
      <label>åå¸æ¶é´</label><input id="tp-date" type="datetime-local" value="${toLocalDTStr(initDate)}">
      <div class="tp-fly-rowfull">å°ä½¿ç¨è®¾ç½®ä¸­çæ¶é´åç§? ${settings.publishTimeOffset || 0} å°æ¶</div>
    `
    ...
    btnOk.addEventListener('click', async () => {
      const title = bodyEl.querySelector('#tp-title').value.trim() || 'æªå½åææ¡?
      ...
      if (cats.length === 0) { alert('åç±»ä¸è½ä¸ºç©º'); return }
      ...
      try {
        ...
        try { ctx.ui?.notice?.(hasCid ? 'æ´æ°æå' : 'åå¸æå', 'ok', 1800) } catch {}
        close()
      } catch (e) {
        console.error(e)
        alert('åå¸å¤±è´¥: ' + (e?.message || e))
      }
    })
  })
}
...
export async function activate(ctx) {
  ...
  ctx.addMenuItem({ label: 'åå¸å?Typecho', title: 'å°å½åææ¡£åå¸å° Typecho (XML-RPC)', onClick: async () => { await openPublishDialog(ctx) } })
}
